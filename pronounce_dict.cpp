...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(word1.begin(), word1.end(), word1copy.begin(), ::toupper);
    std::transform(word2.begin(), word2.end(), word2copy.begin(), ::toupper);

    auto lookup1 = dict.find(word1copy);
    auto lookup2 = dict.find(word2copy);
    if (lookup1 != dict.end() && lookup2 != dict.end()) { // its one of the middle ones
